  ...
  <el-dialog v-model="addBook.show" :title="addBook.title">
    <el-form>
      ...
      <el-form-item label="描述">
        <el-input v-model="addBook.description" :autosize="{ minRows: 4 }" type="textarea" placeholder="请输入描述 不可大于255字 可空" height="40px" ></el-input>
      </el-form-item>
      <el-form-item label="封面">
        <el-input v-model="addBook.cover" placeholder="请输入封面图片地址 可空" ></el-input>
      </el-form-item>
    </el-form>
    <template #footer>
...
export default {
  ...
  methods: {
    ...
    addBook_close(){
      ...
      const that = this;
      API.wordsadmin.books.add(this.addBook.name, this.addBook.description, this.addBook.cover)
        .then((e)=>{
    ...
    delBook(bookid){
      const that = this;
      API.wordsadmin.books.del(bookid)
        .then((e)=>{
          that.$message.success(e.msg);
    ...
    editBook_close(){
      ...
      const that = this;
      API.wordsadmin.books.edit(this.addBook.bookid,this.addBook.name, this.addBook.description, this.addBook.cover)
        .then((e)=>{
          that.$message.success(e.msg);
